...
cd juno
git fetch --tags && git checkout v17.1.1
make install
junod version --long | grep -e version -e commit
#version: v17.1.1
#commit: 48507ed9b83511089cbf1fdc5bae54cae4a7f4b2
...
```python
sed -i.bak -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0ujuno\"/;" ~/.juno/config/app.toml
...
```
# SnapShot Mainnet (~8 GB) updated every 10 hours  
```python
cd $HOME
snap install lz4
